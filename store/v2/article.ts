 ...
 * @param downloaded
 */
export async function updateArticleContentDownload(url: string, downloaded: boolean): Promise<void> {
  db.transaction('rw', 'article', async () => {
    db.article
      .where('link')
      .equals(url)
      .modify(article => {
        // 使用类型断言添加contentDownload字段
        (article as any).contentDownload = downloaded;
      });
  });
}
 ...
 * @param url
 */
export async function getArticleContentDownload(url: string): Promise<boolean> {
  const article = await db.article.where('link').equals(url).first();
  if (article && (article as any).contentDownload !== undefined) {
    return (article as any).contentDownload;
  }
  return false;
}
